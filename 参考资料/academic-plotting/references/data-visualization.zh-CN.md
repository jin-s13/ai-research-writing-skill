...
|---------------|---------------|
| 黑线、无标记 | 彩色线 + 各方法不同标记 |
| 线周围无阴影 | `fill_between(alpha=0.12)` 置信带 |
| 通用蓝柱 | 「Ocean Dusk」色板 + 柱间白边 |
| baseline 同色 | 灰 baseline + coral 高亮「ours」 |
| 虚线网格 | 极淡实线网格（`alpha=0.15`） |
| 默认紧凑边距 | `pad_inches=0.08`、充足轴边距 |
| 柱顶无数值 | 柱上方小号数值 |
| 带框图例 | 无边框图例、放在图内 |
...
ML 论文最常见。展示训练 step 上的 loss/accuracy。
...
## 图型 9：Scaling Law 图（双对数）

LLM 论文中常见的算力/数据/参数量 scaling：
